...
public class TCPClient {
     ...
     */
    public String send(final String message) throws IOException, ClassNotFoundException {
     ...
     * @throws ClassNotFoundException
     */
    public String send(final String message, final byte[] responseTerminator) throws IOException, ClassNotFoundException {
        logger.info(String.format("Sending the message %s.", message));
    ...
    private boolean isTerminated(final byte[] potentialTerminator, final byte[] terminator) {
        return (isEqualByteArray(potentialTerminator, terminator) || (getByteCount() == 1 && (potentialTerminator[0] == ACK || potentialTerminator[0] == NAK)));
    }
